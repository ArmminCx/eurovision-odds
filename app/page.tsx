...
function ShareholdersModal({ country, allVotes, onClose }: { country: any, allVotes: any[], onClose: () => void }) {
  ...
  const [extraUsers, setExtraUsers] = useState<Record<string, { username: string, avatar_url: string }>>({});

  // Fetch usernames from ratings table to fill in for old votes that lack a username
  useEffect(() => {
    async function fetchUsernames() {
      const { data } = await supabase.from('ratings').select('user_id, username, avatar_url')
      if (data) {
        const map: Record<string, any> = {}
        data.forEach((r: any) => { if (r.username) map[r.user_id] = { username: r.username, avatar_url: r.avatar_url } })
        setExtraUsers(map)
      }
    }
    fetchUsernames()
  ...
  // Aggregate votes by user, using ratings data as fallback for missing usernames
  const shareholders = useMemo(() => {
    ...
    countryVotes.forEach(vote => {
      ...
      if (!map.has(uid)) {
        const fallback = extraUsers[uid]
        map.set(uid, {
          username: vote.username || fallback?.username || 'Unknown',
          avatar: vote.avatar_url || fallback?.avatar_url,
          count: 0
        })
      }
      map.get(uid)!.count += 1
  ...
  }, [allVotes, country.id, extraUsers])
...
// --- EDIT COUNTRY MODAL (ADMIN ONLY) ---
function EditCountryModal({ country, onClose, onSave }: { country: any, onClose: () => void, onSave: (data: any) => void }) {
  const [form, setForm] = useState({
    name: country.name || '',
    code: country.code || '',
    artist: country.artist || '',
    song: country.song || '',
    youtube_url: country.youtube_url || '',
  })
    ...
    <div className="fixed inset-0 z-[200] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="glass w-full max-w-md p-6 rounded-2xl border border-white/20 relative" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-5 border-b border-white/10 pb-4">
          <div>
            <h2 className="text-xl font-bold text-white">✏️ Edit Country</h2>
            <p className="text-xs text-gray-400 mt-0.5">Admin only — changes save immediately</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-white transition">✕</button>
        </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Country Name</label>
              <input value={form.name} onChange={e => set('name', e.target.value)} className="w-full bg-white/10 border border-white/10 rounded-lg p-2.5 text-white text-sm outline-none focus:border-pink-500 transition" />
            </div>
            <div>
              <label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Flag Code</label>
              <input value={form.code} onChange={e => set('code', e.target.value.toUpperCase())} maxLength={2} className="w-full bg-white/10 border border-white/10 rounded-lg p-2.5 text-white text-sm outline-none focus:border-pink-500 transition uppercase" />
            </div>
          </div>

          <div>
            <label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Artist</label>
            <input value={form.artist} onChange={e => set('artist', e.target.value)} className="w-full bg-white/10 border border-white/10 rounded-lg p-2.5 text-white text-sm outline-none focus:border-pink-500 transition" />
          </div>

          <div>
            <label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">Song Title</label>
            <input value={form.song} onChange={e => set('song', e.target.value)} className="w-full bg-white/10 border border-white/10 rounded-lg p-2.5 text-white text-sm outline-none focus:border-pink-500 transition" />
          </div>

          <div>
            <label className="text-[10px] text-gray-400 uppercase font-bold block mb-1">YouTube URL</label>
            <input value={form.youtube_url} onChange={e => set('youtube_url', e.target.value)} placeholder="https://youtube.com/watch?v=..." className="w-full bg-white/10 border border-white/10 rounded-lg p-2.5 text-white text-sm outline-none focus:border-pink-500 transition text-blue-300" />
          </div>
        </div>

        <div className="flex gap-2 mt-6">
          <button onClick={() => onSave(form)} className="flex-1 bg-pink-600 hover:bg-pink-500 text-white py-2.5 rounded-xl font-bold transition shadow-lg">💾 Save Changes</button>
          <button onClick={onClose} className="flex-1 bg-white/10 hover:bg-white/20 text-white py-2.5 rounded-xl font-bold transition">Cancel</button>
        </div>
      </div>
    </div>
  )
...
export default function Home() {
  ...
  const handleUpdateCountry = async (form: any) => {
    ...
    if (error) { toast.error('Failed to update') } else { toast.success('Saved!'); setEditingCountry(null); refreshData() }
  }

  // Sort mode
  type SortMode = 'win' | 'rating' | 'az' | 'mine'
  const [sortMode, setSortMode] = useState<SortMode>('win')

  // Rank movement tracking — baseline persisted in localStorage so arrows survive page reloads
  const dailyBaselineRef = useRef<{ date: string, rankMap: Record<number, number> } | null>(null)
  const [rankDeltas, setRankDeltas] = useState<Record<number, number>>({})

  // Load today's saved baseline from localStorage on mount
  useEffect(() => {
    try {
      const saved = localStorage.getItem('esc_rank_baseline')
      if (saved) {
        const parsed = JSON.parse(saved)
        const today = new Date().toDateString()
        if (parsed.date === today) dailyBaselineRef.current = parsed
      }
    } catch { }
  }, [])
  ...
  async function refreshData() {
    ...
    if (vList) {
      ...
      // --- RANK DELTA CALCULATION (daily baseline via localStorage) ---
      if (cList) {
        const total = vList.length
        const getPct = (id: number) => {
          const c = vList.filter((v: any) => v.country_id === id).length
          return total === 0 || c === 0 ? 0 : (c / total) * 100
        }
        const newSorted = [...cList].sort((a, b) => getPct(b.id) - getPct(a.id))
        const newRankMap: Record<number, number> = {}
        newSorted.forEach((c, i) => { newRankMap[c.id] = i + 1 })

        const today = new Date().toDateString()

        if (!dailyBaselineRef.current || dailyBaselineRef.current.date !== today) {
          // First load of the day — save this as the baseline, no arrows yet
          const newBaseline = { date: today, rankMap: newRankMap }
          dailyBaselineRef.current = newBaseline
          try { localStorage.setItem('esc_rank_baseline', JSON.stringify(newBaseline)) } catch { }
        } else {
          // Compare current ranking vs today's baseline
          const deltas: Record<number, number> = {}
          newSorted.forEach(c => {
            const prev = dailyBaselineRef.current!.rankMap[c.id]
            if (prev !== undefined) deltas[c.id] = prev - newRankMap[c.id]
          })
  ...
  if (user) {
    ...
    const filteredCountries = countries.filter(c => {
      ...
      if (sortMode === 'mine') return matchesSearch && myVotes.some(v => v.country_id === c.id)
      return matchesSearch
    })

    const sortedCountries = [...filteredCountries].sort((a, b) => {
      if (sortMode === 'win') return getOddsValue(b.id) - getOddsValue(a.id)
      if (sortMode === 'rating') return parseFloat(getAvgScore(b.id)) - parseFloat(getAvgScore(a.id))
      if (sortMode === 'az') return a.name.localeCompare(b.name)
      if (sortMode === 'mine') return getOddsValue(b.id) - getOddsValue(a.id)
      return 0
    })
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="flex gap-2 flex-wrap justify-center">
              {([
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-xs font-bold border transition ${sortMode === key
                    ? 'bg-pink-600 border-pink-500 text-white shadow-[0_0_12px_rgba(236,72,153,0.4)]'
                    : 'glass border-white/20 text-gray-400 hover:text-white hover:border-white/40'
                    }`}
                >
                  {label}
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            {sortedCountries.map((country, index) => {
              ...
              const odds = getOddsValue(country.id).toFixed(1)
              const isFavorite = index === 0 && allVotes.length > 0 && !searchQuery && sortMode === 'win'
                ...
                <div key={country.id} className={`glass rounded-xl overflow-hidden relative group transition-all duration-500 ${isFavorite ? 'border-yellow-400 shadow-[0_0_30px_rgba(250,204,21,0.4)]' : 'hover:border-pink-500/50'}`}>
                  {user.id === ADMIN_ID && (
                    <div className="absolute top-2 right-2 z-40 flex gap-1.5">
                      <button onClick={() => setEditingCountry(country)} className="bg-blue-600/80 hover:bg-blue-500 p-2 rounded text-white shadow-lg backdrop-blur" title="Edit">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button onClick={() => handleDeleteCountry(country.id, country.name)} className="bg-red-600/80 hover:bg-red-500 p-2 rounded text-white shadow-lg backdrop-blur" title="Delete">
                  ...
                  <div className="h-24 md:h-32 w-full relative overflow-hidden cursor-pointer" onClick={() => videoId ? setActivePostcard(country) : toast.error(t.no_video)}>
                    ...
                    <div className="absolute top-2 left-2 flex items-center gap-1.5">
                      <div className="bg-black/60 backdrop-blur px-2 py-0.5 md:px-3 md:py-1 rounded text-white font-mono font-bold text-xs md:text-base border border-white/10">#{index + 1}</div>
                      {(rankDeltas[country.id] ?? 0) > 0 && (
                        <div className="flex items-center gap-0.5 bg-green-500/20 border border-green-500/50 px-1.5 py-0.5 rounded text-green-400 font-bold text-[10px] md:text-xs animate-fade-in backdrop-blur">
                          <span>▲</span><span>{rankDeltas[country.id]}</span>
                        </div>
                      )}
                      {(rankDeltas[country.id] ?? 0) < 0 && (
                        <div className="flex items-center gap-0.5 bg-red-500/20 border border-red-500/50 px-1.5 py-0.5 rounded text-red-400 font-bold text-[10px] md:text-xs animate-fade-in backdrop-blur">
                          <span>▼</span><span>{Math.abs(rankDeltas[country.id])}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex justify-between items-end mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          ...
                          <span className="text-[10px] text-gray-400 uppercase block">WIN %</span>
                        </div>
                      </div>
                    </div>
